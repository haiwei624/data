...
typedef struct Value{
    ValueType type;
    union{
        IdName id;                   /* if the node represent the access of the identifier */
...
typedef struct Statement{
    StmtType type;
    union{
        IdName variable;              /* print statement */
...
void add_table( SymbolTable *table, char c[], DataType t ,HashTable *Htable);
SymbolTable build( Program program ,HashTable *Htable);
void convertType( Expression * old, DataType type );
DataType generalize( Expression *left, Expression *right );
DataType lookup_table( SymbolTable *table, char c[] ,HashTable *Htable);
void checkexpression( Expression * expr, SymbolTable * table ,HashTable *Htable);
void checkstmt( Statement *stmt, SymbolTable * table ,HashTable *Htable);
void check( Program *program, SymbolTable * table, HashTable *Htable);
void fprint_op( FILE *target, ValueType op );
void fprint_expr( FILE *target, Expression *expr ,HashTable *Htable);
void gencode( Program prog, FILE * target ,HashTable *Htable);
