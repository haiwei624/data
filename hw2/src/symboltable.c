...
void printFreq()
{
    ...
    int j;
    int count = 0;
    symtab * sorted_table[TABLE_SIZE];
    int min;
    symtab * tmp;
    for (i=0; i<TABLE_SIZE; i++)
    {
    ...
    for(i=0; i<count; i++)
    {
    	...
    	for(j=i+1; j<count; j++)
	    {
	    	if(strcmp(sorted_table[j]->lexeme, sorted_table[min]->lexeme) < 0)
	    	{
	    		min = j;
	    	}
	    }
	    tmp = sorted_table[i];
	    sorted_table[i] = sorted_table[min];
	    sorted_table[min] = tmp;
    }
    for(i=0; i<count; i++)
    {
    	printf("%s\t\t%d\n", sorted_table[i]->lexeme, sorted_table[i]->counter);
    }
}
